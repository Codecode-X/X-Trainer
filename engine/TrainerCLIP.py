...
class TrainerClip(TrainerBase):
    ...
    def init_model(self, cfg):
        ...
        sorted_labels = sorted(self.lab2cname.items(), key=lambda x: x[0]) # 将文本标签按照 label 从小到大排序，方便模型预测结果与label进行对齐
        label_texts = [item[1] for item in sorted_labels]  # 文本标签tensor | [num_classes]
    ...
    def forward_backward(self, batch): 
        ...
        assert image is not None and label is not None, "forward_backward()中parse_batch_train解析到的图像和标签不能为空"

        prec = self.cfg.TRAINER.PREC  # 配置的精度
        if prec == "amp":  # 自动混合精度训练
            with autocast():
                # Clip需要传入 图像 和 文本(初始化模型时已经加载了每个类别的文本特征)。
